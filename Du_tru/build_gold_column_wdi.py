...
Tách "dự trữ vàng" thành cột riêng biệt từ World Bank WDI API:
- Total reserves (includes gold): FI.RES.TOTL.CD
- Total reserves minus gold: FI.RES.XGLD.CD
- Gold reserves (inferred): = Total - Non-Gold
...
# Output files
OUTPUT_CSV = "reserves_gold_split_wdi.csv"
OUTPUT_PARQUET = "reserves_gold_split_wdi.parquet"
...
def fetch_countries() -> List[Dict]:
    ...
    while True:
        ...
        logger.info(f"Page {page}: {len(countries_page)} fetched (kept {len(all_countries)} after filtering)")
...
def fetch_indicator_data(indicator_id: str, countries: List[Dict]) -> pd.DataFrame:
    ...
    while True:
        ...
        logger.info(f"Page {page}: {len(records_page)} records (processed: {len(all_records)} with non-null values)")
...
def calculate_gold_inference(df_total: pd.DataFrame, df_non_gold: pd.DataFrame) -> pd.DataFrame:
    """
    Join total and non-gold reserves, then calculate inferred gold value with quality flags.
    ...
    merged["gold_reserves_value_usd_inferred"] = merged["value_total"] - merged["value_non_gold"]

    # Add quality flags based on business logic
    def assign_quality_flag(row):
        gold_val = row.get("gold_reserves_value_usd_inferred")
        total_val = row.get("value_total")

        if pd.isna(gold_val):
            return "MISSING_DATA"
        elif gold_val < 0:
            return "NEGATIVE_GOLD_INFERRED"
        elif gold_val == 0 and pd.notna(total_val) and total_val > 0:
            return "GOLD_ZERO_OR_NOT_REPORTED"
        else:
            return "OK"

    merged["quality_flag"] = merged.apply(assign_quality_flag, axis=1)
    ...
    columns = [
        ...
        "total_reserves_usd", "non_gold_reserves_usd", "gold_reserves_value_usd_inferred",
        "quality_flag"
    ]
    merged = merged[columns]

    # Count quality flags
    quality_counts = merged["quality_flag"].value_counts()
    logger.info(f"Quality flag distribution:\n{quality_counts.to_string()}")

    return merged


def generate_coverage_report(df_total: pd.DataFrame, df_non_gold: pd.DataFrame,
    ...
    for iso2 in sorted(all_iso2):
        ...
        # Get year range for records with both indicators
        both_data = df_merged[
            (df_merged["iso2"] == iso2) &
            (df_merged["total_reserves_usd"].notna()) &
            (df_merged["non_gold_reserves_usd"].notna())
        ]

        if not both_data.empty:
            first_year = int(both_data["year"].min())
            last_year = int(both_data["year"].max())
            n_years = len(both_data["year"].unique())
        else:
            first_year = None
            last_year = None
            n_years = 0
...
def print_summary_statistics(df_total: pd.DataFrame, df_non_gold: pd.DataFrame,
    ...
    gold_data = df_merged[
        df_merged["gold_reserves_value_usd_inferred"].notna()
    ...
    quality_counts = df_merged["quality_flag"].value_counts()
    logger.info(f"\nQuality flag distribution:")
    ...
    if len(missing) == 0:
        logger.info("  None - all economies have both indicators!")
    else:
...
def export_data(df_merged: pd.DataFrame, df_coverage: pd.DataFrame):
    ...
    try:
        ...
        logger.info(f"Also exported to {OUTPUT_PARQUET}")
    except ImportError:
...
def main():
    ...
    logger.info("="*80)
    logger.info("WORLD BANK WDI - GOLD RESERVES COLUMN BUILDER")
    logger.info("="*80 + "\n")
